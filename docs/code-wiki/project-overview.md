...
| 数据库 | 运行时可选 MongoDB/Mongoose、SQLite/Drizzle、MySQL/Drizzle | `lib/database/config.ts`；`lib/database/repositories.ts`；`lib/database/sql/*` |
...
| `lib/database/` | provider 配置、Repository、SQL schema/连接 | `lib/database/config.ts`、`repositories.ts`、`sql/*` |
| `lib/models/` | MongoDB adapter/迁移源模型 | `lib/models/submission.ts:1-44`、`config.ts:1-14` |
...
| `/api/admin/settings` | 管理员读取/写入当前数据库配置 |
...
| 术语 | 含义 |
|---|---|
| `Submission` | 当前数据库中的一条友链申请或更新申请记录 |
| `apply` | 新增友链申请类型 |
...
| `Config` | 当前数据库中的键值配置，保存清理策略、邮件模板和 OwO 地址 |
